...
class StardewLogic(ReceivedLogicMixin, HasLogicMixin, RegionLogicMixin, TravelingMerchantLogicMixin, TimeLogicMixin,
    ...
    def __init__(self, player: int, options: StardewValleyOptions, content: StardewContent, regions: Collection[str]):
        ...
        self.registry.item_rules.update({
            ...
            Geode.omni: self.count(2, *(self.region.can_reach_all(Region.oasis, Region.sewer), self.region.can_reach(Region.mines_floor_100), self.region.can_reach(Region.skull_cavern_25), self.region.can_reach(Region.volcano_floor_5), (self.region.can_reach_all(Region.island_west, Region.island_north,) & self.has(Consumable.treasure_totem)))),  # Could add self.fish_pond.can_get_fish_pond_reward(Fish.octopus, 9, Geode.omni) but it makes a logic loop. Same for self.tool.has_pan(ToolMaterial.iron)
