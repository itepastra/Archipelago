...
def create_region(world: UnfairFlipsWorld, name: str, region_type: str, gate_index=None):
    ...
    if gate_index is not None:
        create_locations(world, reg, region_type, gate_index)
...
def connect_all_regions(world: UnfairFlipsWorld):
    for gate_index in range(math.ceil(world.options.required_heads / 2)):
        ...
        if gate_index == 0:
            ...
            fairness_entrance = connect_regions(world, f"Fairness Gate {gate_index}", f"Fairness Gate {gate_index + 1}", f"Fairness Gate {gate_index} -> Fairness Gate {gate_index + 1}" )
            shop_entrance = connect_regions(world, f"Shop Gate {gate_index}", f"Shop Gate {gate_index + 1}", f"Shop Gate {gate_index} -> Shop Gate {gate_index + 1}")
        fairness_entrance.access_rule = lambda state, gate=gate_index: state.has(f"Progressive Fairness", world.player, gate) and state.has(f"Heads+", world.player, max(gate - 1, 0))
